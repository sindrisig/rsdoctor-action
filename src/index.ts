...
(async () => {
  try {
    ...
    if (isPR && projectReports.length > 0) {
      ...
      // Generate summary table for quick overview (only useful for multiple projects)
      if (reportsWithCurrent.length > 1) {
      ...
      } else if (reportsWithCurrent.length === 1 && reportsWithCurrent[0].current) {
        // Single project with no changes: still show the metrics table inline
